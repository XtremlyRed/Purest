...
public class EventManager : IEventManager
{
    ...
    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    ConcurrentDictionary<Type, ConcurrentDictionary<string, object>> asyncEventStorage = new();
    ...
    /// <summary>
    /// get async <see langword="event"/> <see langword="by"/> <typeparamref name="TEvent"/>
    /// </summary>
    /// <typeparam name="TEvent"></typeparam>
    /// <returns></returns>
    public IEventAsync<TEvent> GetEventAsync<TEvent>()
    {
        return GetEventAsync<TEvent>(commonKey);
    }
    ...
    /// <returns></returns>
    /// <exception cref="NotImplementedException"></exception>
    public IEventAsync<TEvent> GetEventAsync<TEvent>(string channelName)
    {
        var eventType = typeof(TEvent);
        if (asyncEventStorage.TryGetValue(eventType, out var channelStorage) == false)
        {
            lock (asyncEventStorage)
            {
                if (asyncEventStorage.TryGetValue(eventType, out channelStorage) == false)
                {
                    asyncEventStorage[eventType] = channelStorage = new ConcurrentDictionary<string, object>();
                }
            }
        }
        ...
        if (channelStorage.TryGetValue(channelName, out var eventChannel) == false)
        {
            lock (asyncEventStorage)
            {
                ...
                {
                    var weak = new WeakReference(channelStorage);
                    channelStorage[channelName] = eventChannel = new EventChannelAsync<TEvent>(weak, channelName);
                }
            }
        }

        if (eventChannel is not IEventAsync<TEvent> @event)
        {
            throw new InvalidOperationException($"channel name : {channelName} may be registered by other types");
        }
