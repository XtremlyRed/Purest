...
/// <summary>
/// a <see langword="class"/> of <see cref="EventChannelAsync{TEvent}"/>
/// </summary>
/// <typeparam name="TEvent"></typeparam>
internal class EventChannelAsync<TEvent> : IEventAsync<TEvent>
{
    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    WeakReference weakReference;
    ...
    [DebuggerBrowsable(DebuggerBrowsableState.Never)]
    Func<TEvent, Task>? subscribe;

    internal EventChannelAsync(WeakReference weakReference, string channelName)
    {
        this.weakReference = weakReference;
        this.channelName = channelName;
    }
    ...
    /// <param name="subscribe"></param>
    /// <returns></returns>
    public IUnsubscriber Subscribe(Func<TEvent, Task> subscribe)
    {
        this.subscribe = subscribe;
        return new Unsubscriber(this.weakReference, this.channelName)!;
    }
